<script setup lang="ts">
    ...
    import AuthMini from "./AuthMini.vue";
    ...
    const Start = () => {
        ...
        board.value.initCells()
        if(!currentPlayer.value?.name && getPlayerStore.value.name) currentPlayer.value = new Player(getPlayerStore.value.name, getPlayerStore.value.record, getPlayerStore.value.lastGame);
...
<template>
  <div @click="closePopupOutsideClick">
    <AuthMini v-if="width < 768" :Exit :isGameStarted @handel-click="isHiddenLogin = false" />
